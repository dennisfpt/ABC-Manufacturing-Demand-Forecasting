...
# ── Train ─────────────────────────────────────────────────────────────────────
# Dữ liệu dùng để train model = đúng Brand + Category đang được chọn ở sidebar
# (trước đây bị cố định luôn là Samsung dù đổi "Compare Brand" thế nào cũng vậy -> đã sửa)
brand_cat_data = df_all[(df_all["ProductBrand"] == sel_brand) & (df_all["ProductCategory"] == sel_cat)]
base_price = brand_cat_data["ProductPrice"].mean() if len(brand_cat_data) > 0 else 100.0

# GỌI HÀM PIPELINE VỚI DỮ LIỆU ĐỘNG THEO BRAND + CATEGORY ĐANG CHỌN
series, results, fc, xgb_model, X_train = run_entire_forecasting_pipeline(brand_cat_data)
...
with k3:
    intent_val = brand_cat_data['PurchaseIntent'].mean()*100 if len(brand_cat_data) > 0 else 0
...
with k4:
    sat_val = brand_cat_data['CustomerSatisfaction'].mean() if len(brand_cat_data) > 0 else 0
...
# ── Forecast Chart ────────────────────────────────────────────────────────────
st.markdown(f"<div class='sh'>📈 Demand Forecast - {sel_brand} {sel_cat}</div>", unsafe_allow_html=True)
fig_fc = go.Figure()
...
with col8:
    st.markdown(f"**3-Month Forecast - {sel_brand} {sel_cat}**")
    fc_df = pd.DataFrame({
...
recs = [
    ...
    f"**Purchase Intent is {intent_val:.0f}%** for {sel_brand} {sel_cat} - prioritize inventory.",
